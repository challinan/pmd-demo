...
DataSupplier::DataSupplier(QObject* parent): QObject(parent)
{
     ...
     connect(timer, SIGNAL(timeout()), this, SLOT(tickerUpdate()));
     timer-> start(30);
}
...
void DataSupplier::tickerUpdate()
{
    ...
    pm_data.ecg=getECGData();
    pm_data.abp=getABPData();
    pm_data.pleth=getPlethData();
